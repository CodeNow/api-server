...
var me = module.exports = createMongooseMiddleware(User, {
  ...
  changepass: function (req, res, next) {
    series(
      body.pickAndRequire('password', 'passwordNew'),
      this.findMe,
  ...
  changeMailReq: function (req, res, next) {
    ...
    function ensureNoConflict (req, res, next) {
      req.searchEmail = req.body.email_new;
      me.findConflictEmail(req, res, next);
    }
    function registeredFields (req, res, next) {
      req.body.email_new = req.body.email_new.toLowerCase();
  ...
  findConflictEmail: function (req, res, next) {
    var query = { email: req.searchEmail.toLowerCase() };
    User.findOne(query, { _id:1, email:1 }, req.domain.intercept(function (user) {
      if (user) {
        if ( user.email === req.searchEmail.toLowerCase() ) {
          next( error(409, 'user with this email already registered') );
        }
      }
      else {
        next();
  ...
  login: function (loginData) {
    return series(
      ...
      body.pick('username', 'email', 'password', 'containerID'),
      this.findOne({
        $or: [
          { email: 'body.email' },
          { username: 'body.username' },
      ...
      logUser,
      body.pick('last_login', 'last_login_ip'),
      this.model.setAndSave('body')
    );

    function logUser(req, res, next) {
      req.body.last_login = Date();
      req.body.last_login_ip = req.headers['x-real-ip'];
      next();
    }
  },
