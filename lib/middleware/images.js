...
var images = module.exports = createMongooseMiddleware(Image, {
  ...
  createArchiveCopy: function(imageKey) {
    var archiveImages = require('middleware/archiveImages');
    return series(
      archiveImages.createFromImage(imageKey)
      // archiveImages.save()
    );
  },
  reportAbuse: function(imageKey) {
    return series(
      this.sendAbuseReport
    );
  },
  sendAbuseReport: function (req, res, next) {
    emailer.sendAbuseReport(path.join(utils.encodeId(req.image._id), utils.urlFriendly(req.image.name)),
                            req.me, next);
  },
  ...
  findChildContainers: function (imagekey) {
    var containers = require('middleware/containers');
    return function (req, res, next) {
      containers.find({
        'parent': req[imagekey]._id },
        'containerId host',
        function (err, docs) {
          if(!err) { req.childContainers = docs; }
          next();
      })(req, res, next);
    };
  },
  deleteChildContainers: function (imagekey) {
    var containers = require('middleware/containers');
    return function (req, res, next) {
      containers.remove({'parent': req[imagekey]._id })(req, res, next);
    };
  }
});
