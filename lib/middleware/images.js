...
var images = module.exports = createMongooseMiddleware(Image, {
  ...
  reportSpam: function(imageKey) {
    return series(
      this.sendSpamReport
    );
  },
  sendSpamReport: function (req, res, next) {
    emailer.sendSpamReport(path.join(utils.encodeId(req.image._id), utils.urlFriendly(req.image.name)),
                            req.me, next);
  },
