...
function readConfigs (filename) {
  ...
  var checkConfigs = ["tokenExpires", "passwordSalt"];
  for (var i = checkConfigs.length - 1; i >= 0; i--) {
    if(!config[checkConfigs[i]]) {
      throw new Error("missing required config!: "+checkConfigs[i]);
    }
  };
