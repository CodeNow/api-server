...
var importer = require('middleware/importer');
...
// TODO: updateContainer needs rework, especially the commit interactions with harbourmaster..
  ...
  lll = function(req, res, next) {
      ...
      console.log("res: "+JSON.stringify(res, function(key, value) {
        if (typeof value === 'object' && value !== null) {
...
///NEW
  var importFromUrl =
    series(
      ...
      ll("checkFound"),
      images.fetchChannelImage('channel'),
      //images.findById('query.image'),
      ll("image found"),
      images.checkFound,
      ll("checkFound found"),
      containers.create({ owner: 'user_id' }),
      ll("created"),
      query.setFromParams('owner', 'user_id'),
      ll("setFromParams"),
      containers.model.inheritFromImage('image'),
      ll("inheritFromImage"),
      harbourmaster.createContainer,
      ll("createContainer"),
      harbourmaster.startContainer,
      ll("startContainer"),
      containers.model.save(),
      ll("saved"),
      dockworker.runImportCommands,
      ll("runImportCommands"),
      containers.respond
    );
      


    // find IMAGE  (use clean as default)
    //startContainer();
    // parse url run import files
    // show user container
  app.post('/import/:url', importFromUrl);
