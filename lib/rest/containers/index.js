...
// TODO: updateContainer needs rework, especially the commit interactions with harbourmaster..
  ...
  var getContainerForStart = series(
    params.isObjectId64('containerId'),
    params.decodeId('containerId'),
    mw.query().ifExists('servicesToken')
      .else(
  ...
  var cleanContainerPort = series(
    getContainerForStartStop,
    harbourmaster.killContainer('container')
  );

  var deleteContainer = series(
    containers.findById('params.containerId', { _id: 1 , owner:1 }),
    containers.checkFound,
    or(me.isOwnerOf('container'), me.isModerator),
    containers.removeById('params.containerId')
  ...
  app.put(path.join(baseUrl, ':containerId', 'start'),
    getContainerForStart,
    containers.getOwnerPermissions('container'),
    containers.createContainerIfNotYet,
    containers.updateTermAccess,
  ...
  app.get(path.join(baseUrl, ':containerId', 'extendSession'),
    getContainerForStartStop,
    harbourmaster.extendContainerLife('container'),
    utils.message('ExtendedLife.')
  );

  app.get(path.join(baseUrl, ':containerId', 'killMe'),
    cleanContainerPort,
    deleteContainer,
    utils.message('You are killed.')
  );

  app.get(path.join(baseUrl, ':containerId', 'howMuchLife'),
    getContainerForStartStop,
    harbourmaster.howMuchLife('container'),
    utils.message('Remaining Time.')
  );
