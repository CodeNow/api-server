...
app.get('/',
   ...
   *  Made sure that saved projects with non-registered users will also be deleted
   *  Technically, this scenario should not be possible
   *  Even if it is, it should not be a huge leak of containers
   */
...
function filterContainersWithRegisteredOwner (req, res, next) {
  req.containers = req.containers.filter(function (container) {
    ...
    return containerJSON.ownerJSON.registered;
  });
  next();
}
