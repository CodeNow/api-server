...
var getusers = function (req, res, next) {
  ...
  if (req.query._id || req.query.ids) {
    var userIds = [].concat(req.query._id || req.query.ids);
...
var getuser = function (req, res) {
  users.findUser(req.domain, { _id: req.user_id }, req.domain.intercept(function (user) {
    if (!user) {
      ...
      delete json_user.password;
      async.parallel({
        imagesCount: images.count.bind(images, { owner: user._id }),
        taggedImagesCount: images.count.bind(images, {
          owner: user._id,
          tags: { $not: { $size: 0 } }
        }),
      }, req.domain.intercept(function (results) {
        _.extend(json_user, results);
        res.json(json_user);
      }));
    }
  }));
