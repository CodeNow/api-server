...
function findDockerfile (req, res, next) {
  fs.exists(req.tmpdir + '/Dockerfile', function (exists) {
    if (exists) {
      ...
      fs.readdir(req.tmpdir, function (err, files) {
        if (err) { return next(err); }
...
function loadDockerfile (req, res, next) {
  fs.readFile(req.dockerdir + '/Dockerfile', 'utf8', function (err, dockerfile) {
    if (err) { return next(err); }
...
function readTempFiles (req, res, next) {
  ...
  fs.readdir(req.dockerdir + '/src', function (err, filenames) {
    if (err) { return next(err); }
