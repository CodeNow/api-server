...
function createRequest(method, container, path, name, isDir, opts) {
  var reqOpts = {};
  reqOpts.url = formatURl(container, path, name, isDir);
  reqOpts.method = method;
  reqOpts.json = {};
  if(opts && typeof opts.body === 'object')  {
    reqOpts.json = opts.body;
  }
  reqOpts.json.container = {
    root: container.containerId
  };
  if (opts && typeof opts.query === 'object') {
    reqOpts.qs = opts.query;
  }
...
var volumes = {
  createFs: function (container, data, cb) {
    ...
    request(
      createRequest('POST', container, data.path, data.name, data.dir, opts),
      function (err, res) {
        if (err) {
          cb(err);
  ...
  readFile: function (container, name, path, cb) {
    ...
    request(
      createRequest('POST', container, path, name, false),
      function (err, res) {
        if (err) {
          cb(err);
        } else if (res.statusCode === 502) {
  ...
  moveFile: function (container, name, path, newPath, cb) {
    ...
    request(
      createRequest('POST', container, path, name, false, {
        body: {
          newPath: pathModule.join(container.file_root, newPath, name)
        }
      }), function (err, res) {
        if (err) {
          cb(err);
  ...
  removeFs: function (container, data, cb) {
    ...
    request(
      createRequest('DELETE', container, data.path, data.name, data.dir, opts),
      function (err, res) {
        if (err) {
          cb(err);
        } else if (res.statusCode === 502) {
  ...
  readAllFiles: function (container, cb) {
    request(
      createRequest('GET', container, '', '', true,  {
        query: {
          recursive: "true"
        }
      }), function (err, res) {
  ...
  readDirectory: function (container, subDir, cb) {
    request(
      createRequest('GET', container, subDir, '', true),
      function (err, res) {
        if (err) {
          cb(err);
        } else if (res.statusCode === 502) {
