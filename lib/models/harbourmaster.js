...
Harbourmaster.prototype.startContainer = function (domain, container, token, cb) {
  var opts = {
    json: container,
    headers: { 'runnable-token': token }
  };
  var path = p.join('/containers/', container.servicesToken, '/start');
  this.post(path, opts, domain.intercept(function (res, body) {
    if (res.statusCode !== 204) {
      var code = res.statusCode >= 500? 502 : res.statusCode;
      cb(error(code, 'error starting (code:'+res.statusCode+') - '+JSON.stringify(body)));
    } else {
      cb(null, res);
    }
  }));
