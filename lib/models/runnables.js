...
var Runnables = {
  ...
  listContainers: function (domain, userId, query, cb) {
    query = query || {};
    query.owner = userId;
    containers.find(query, {files:0}, domain.intercept(function (containers) {
  ...
  updateContainer: function (domain, userId, runnableId, updateSet, token, cb) {
    runnableId = decodeId(runnableId);
    var commit = function (container, updateSet, cb) {
      async.waterfall([
        function (cb) {
          // state matters here so using atomic mongo update
          var query = {
            _id: container._id,
            $or: [
          ...
          containers.findOneAndUpdate(query, update, opts, cb);
        },
        function (container, cb) {
          if (!container) { // commit already in progress, or done with no errors
            delete updateSet.status; // don't save the status in the update below, since it was blocked
            return cb();
          }
          var json;
        ...
        async.series([
          function (cb) {
            if (updateSet.status === 'Committing new') {
              images.findOne({ name: updateSet.name || container.name }, { _id: 1 }, domain.intercept(function (existing) {
                if (existing) {
                  cb(error(403, 'a shared runnable by that name already exists'));
                } else {
                  commit(container, updateSet, cb);
                }
              }));
            } else if (updateSet.status === 'Committing back') {
              commit(container, updateSet, cb);
              images.findOne({ _id: container.parent }, { tags: 1 }, domain.intercept(function (image) {
                // send an email to the user, if the container has no more channels
                if (container.tags.length <= 0 && image.tags.length > 0) {
                  users.findUser(domain, { _id: userId }, domain.intercept(function (user) {
                    if (!user) {
                      cb(error(404, 'user not found'));
                    }
                    emailer.delistEmail(user, container);
                  }));
                }
              }));
            } else {
  ...
  listAll: function (domain, sort, limit, page, cb) {
    ...
    query = images.find({tags: { $not: { $size: 0 } }}, listFields).sort(sort).skip(page * limit).limit(limit);
    countQuery = images.find({tags: { $not: { $size: 0 } }}, listFields).sort(sort).skip(page * limit).limit(limit).count();
    async.parallel({
      images: function (cb) {
  ...
  listFiltered: function (domain, query, sort, limit, page, fields, cb) {
    ...
    var countQuery = images.find(query).skip(page * limit).limit(limit).count();
...
var fetchContainer = function (domain, userId, runnableId, cb) {
  ...
  async.parallel({
    ...
    container: function (cb) {
      containers.findOne({ _id: runnableId }, cb);
    }
  },
  domain.intercept(function (results) {
    var container = results.container;
